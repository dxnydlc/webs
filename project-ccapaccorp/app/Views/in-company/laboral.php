
<?= $this->extend('layouts/principal') ?>



<!-- ************************************************** -->
<?= $this->section('titulo') ?>
Laboral
<?= $this->endSection() ?>
<!-- ************************************************** -->





<!-- ************************************************** -->
<?= $this->section('header') ?>
CcapaCcorp
<?= $this->endSection() ?>
<!-- ************************************************** -->


<!-- ************************************************** -->
<?= $this->section('header-class') ?>
main-header-two
<?= $this->endSection() ?>
<!-- ************************************************** -->






<!-- ************************************************** -->
<?= $this->section('content') ?>

<!-- ============================================================== -->


<div class="stricky-header stricked-menu main-menu main-header-two">
        <div class="sticky-header__content"></div><!-- /.sticky-header__content -->
</div><!-- /.stricky-header -->

<section class="page-header page-header--bg-two"  >
    <div class="page-header__bg jarallax-img" style="background-image: url(/2024/banner-09.png);" ></div><!-- /.page-header-bg -->
    <div class="page-header__overlay" style="background-color:rgba(var(--eduact-black2-rgb), 0.5)" ></div><!-- /.page-header-overlay -->
    <div class="container text-center">
        <h2 class="page-header__title" style="text-transform: none;" >laboral</h2>
        <!-- /.page-title -->
        <!-- /.page-breadcrumb list-unstyled -->
    </div><!-- /.container -->
</section><!-- /.page-header -->

<!-- course-details-start -->
<section class="course-details">
    <div class="container">
        <div class="row">
            <div class="col-xl-12 ">
                <div class="course-details__thumb">
                    <!-- <img src="/2024/curso-banner-02.jpeg" alt="eduact" style="max-width: 500px;" /> -->
                </div><!-- details-thumb -->
                <div class="course-details__meta">
                </div>
                <!-- details-meta -->
                <div class="course-details__tabs tabs-box">
                    
                    <!-- tab-title -->
                    <!-- tab-content -->
                    <div class="course-details__overview">
                        <p class="course-details__overview__text" style="font-size:20px;margin-bottom: 3px;" >
                        La capacitación In Company se desarrolla en base a las necesidades de cada empresa. 
                        En tal sentido, escuchamos las necesidades de nuestros clientes, definimos el objetivo y 
                        brindamos la capacitación en materia laboral a la medida de la 
                        necesidad de cada organización.
                        <br/><br/>
                        Los temas más requeridos por nuestros clientes son:
                        </p>

                        <ul class="list-unstyled course-details__overview__lists">
                            <li>
                                <span class="icon-check"></span>
                                Planeamiento de contratación laboral y correcta desvinculación.
                            </li>
                            <li>
                                <span class="icon-check"></span>
                                Planeamiento de contratación de personal a través de empresas de intermediación laboral y/o tercerizadoras.
                            </li>
                            <li>
                                <span class="icon-check"></span>
                                Elaboración del Reglamento Interno de Trabajo.
                            </li>
                            <li>
                                <span class="icon-check"></span>
                                Implementación de las obligaciones frente al hostigamiento sexual laboral.
                            </li>
                            <li>
                                <span class="icon-check"></span>
                                Implementación del sistema de seguridad y salud en el trabajo.
                            </li>
                        </ul>
                    </div>
                </div><!-- tabs -->
            </div>
            
        </div>
    </div>
</section>
<!-- course-details-end -->


<!-- ============================================================== -->

<?= $this->endSection() ?>
<!-- ************************************************** -->

